...
_(Rotas desconhecidas ou a raiz `/` redirecionam automaticamente para `/dashboard/home`)_
...
> _Aviso: Web Push e Service Workers não funcionam corretamente no `ng serve`. Para testar o PWA, faça o build de produção e utilize HTTPS._
...
_(Para descriptografar o backup gerado localmente: `gpg --batch --yes --passphrase SUA_BACKUP_PASSPHRASE -d db.sql.gz.gpg > db.sql.gz`)_
...
_(Definidos em `src/app/shared/models/consts.ts`)_
...
Software de uso exclusivo da Igreja Batista Filadélfia.
